...
/* package */ final class ExperimentalFfmpegVideoDecoder
  ...
  public ExperimentalFfmpegVideoDecoder(
    ...
    nativeContext = ffmpegInitialize(codecName, extraData, threads, degree, width, height);
    if (nativeContext == 0) {
   ...
   */
  @Nullable
  private static byte[] getExtraData(String mimeType, @Nullable List<byte[]> initializationData) {
  ...
  protected FfmpegDecoderException decode(
    ...
    if (sendPacketResult == VIDEO_DECODER_ERROR_INVALID_DATA) {
      ...
      return new FfmpegDecoderException("ffmpegSendPacket error (see logcat)");
    }
    ...
    if (getFrameResult == VIDEO_DECODER_ERROR_OTHER) {
      return new FfmpegDecoderException("ffmpegReceiveFrame error (see logcat)");
    }

    if (getFrameResult == VIDEO_DECODER_ERROR_READ_FRAME
        || getFrameResult == VIDEO_DECODER_ERROR_INVALID_DATA) {
      outputBuffer.shouldBeSkipped = true;
    }
  ...
  public void renderToSurface(VideoDecoderOutputBuffer outputBuffer, Surface surface)
    ...
    if (rst == VIDEO_DECODER_ERROR_OTHER) {
      throw new FfmpegDecoderException("Buffer render error (see logcat)");
    }
  }
  ...
  private native int ffmpegSendPacket(long context, ByteBuffer encodedData, int length, long inputTimeUs);
