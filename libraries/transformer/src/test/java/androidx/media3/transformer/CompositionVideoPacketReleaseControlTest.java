...
public class CompositionVideoPacketReleaseControlTest {
  ...
  private ImmutableList<AsyncFrame> firstPacket;
  ...
  public void onRender_releaseActionDrop_releasesFrame() throws ExoPlaybackException {
    ...
    assertOutputEvents(/* ignoreReleaseTime= */ true, toFramesEvent(firstPacket));
    assertThat(releasedFrameTimestamps).containsExactly(packet.get(0).frame.getContentTimeUs());
  }
  ...
  public void onRender_releaseActionSchedule_forwardsFrameDownstreamWithReleaseTime()
    ...
    ImmutableList<AsyncFrame> expectedPacket =
    ...
    ImmutableList<AsyncFrame> expectedFirstFrame =
  ...
  public void queue_lowerPresentationTimeButHigherSequenceTime_forwardsFrameDownstream()
    ...
    ImmutableList<AsyncFrame> expectedPacket1 =
        updatePacketWithReleaseTime(packet1, /* releaseTimeNs= */ 120_000_000);
    ImmutableList<AsyncFrame> expectedPacket2 =
  ...
  public void flushPrimarySequence_releasesHeldFramesAndResetsReleaseControl()
    ...
    assertThat(releasedFrameTimestamps)
        .containsExactly(
            packet1.get(0).frame.getContentTimeUs(), packet2.get(0).frame.getContentTimeUs())
        .inOrder();
    ...
    assertThat(releasedFrameTimestamps)
        .containsExactly(
            packet1.get(0).frame.getContentTimeUs(),
            packet2.get(0).frame.getContentTimeUs(),
            firstPacket.get(0).frame.getContentTimeUs())
        .inOrder();
  }
  ...
  public void queue_eosPacket_doesNotIsEndedWhenOnRenderNotCalled() {
    compositionVideoPacketReleaseControl.queue(ImmutableList.of(END_OF_STREAM_ASYNC_FRAME));
    assertThat(compositionVideoPacketReleaseControl.isEnded()).isFalse();
  }
  ...
  public void onRender_normalPlaybackAndRedraw_managesRefCountsCorrectly() throws Exception {
    compositionVideoPacketReleaseControl.onStarted();
    ImmutableList<AsyncFrame> packet0 =
        createPacket(/* presentationTimeUs= */ 100_000, /* sequencePresentationTimeUs= */ 100_000);
    ImmutableList<AsyncFrame> packet1 =
        createPacket(/* presentationTimeUs= */ 200_000, /* sequencePresentationTimeUs= */ 200_000);
  ...
  private static FramesEvent toFramesEvent(List<AsyncFrame> packet) {
    return new FramesEvent(ImmutableList.copyOf(packet));
  }
  ...
  private void assertOutputEvents(boolean ignoreReleaseTime, Event... expectedEvents) {
    ...
    for (int i = 0; i < expectedEvents.length; i++) {
      ...
      if (expectedEvent instanceof FramesEvent) {
        FramesEvent framesEvent = (FramesEvent) expectedEvent;
        assertThat(receivedEvent).isInstanceOf(FramesEvent.class);
        ImmutableList<AsyncFrame> receivedFrames = ((FramesEvent) receivedEvent).frames;
        ImmutableList<AsyncFrame> expectedFrames = framesEvent.frames;
  ...
  private static ImmutableList<AsyncFrame> updatePacketWithReleaseTime(
      ImmutableList<AsyncFrame> packet, long releaseTimeNs) {
    ImmutableList.Builder<AsyncFrame> updatedPacketBuilder = new ImmutableList.Builder<>();
    for (AsyncFrame asyncFrame : packet) {
      DefaultHardwareBufferFrame commonFrame = (DefaultHardwareBufferFrame) asyncFrame.frame;
      ImmutableMap.Builder<String, Object> metadataBuilder =
          ImmutableMap.<String, Object>builder().putAll(commonFrame.getMetadata());
      metadataBuilder.put(Frame.KEY_DISPLAY_TIME_NS, releaseTimeNs);
      DefaultHardwareBufferFrame updatedCommonFrame =
          new DefaultHardwareBufferFrame.Builder(commonFrame.getHardwareBuffer())
              .setFormat(commonFrame.getFormat())
              .setContentTimeUs(commonFrame.getContentTimeUs())
              .setMetadata(metadataBuilder.buildKeepingLast())
              .setInternalImage(commonFrame.getInternalImage())
              .build();
      updatedPacketBuilder.add(new AsyncFrame(updatedCommonFrame, asyncFrame.acquireFence));
    }
