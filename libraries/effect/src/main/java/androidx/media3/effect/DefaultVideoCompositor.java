...
public final class DefaultVideoCompositor implements VideoCompositor {
  ...
  @Override
  public synchronized void registerInputSource(@IntRange(from = 0) int sequenceIndex) {
    checkState(!contains(inputSources, sequenceIndex));
    inputSources.put(sequenceIndex, new InputSource());
  }

  @Override
  public synchronized void signalEndOfInputSource(int sequenceIndex) {
    inputSources.get(sequenceIndex).isInputEnded = true;
    ...
    if (inputSources.get(PRIMARY_SEQUENCE_INDEX).frameInfos.isEmpty()) {
      if (sequenceIndex == PRIMARY_SEQUENCE_INDEX) {
    ...
    if (sequenceIndex != PRIMARY_SEQUENCE_INDEX
        && inputSources.get(sequenceIndex).frameInfos.size() == 1) {
  ...
  public synchronized void queueInputTexture(
    ...
    InputSource inputSource = inputSources.get(sequenceIndex);
    ...
    InputFrameInfo inputFrameInfo =
        new InputFrameInfo(
            ...
            inputTexture,
            presentationTimeUs,
            settings.getOverlaySettings(sequenceIndex, presentationTimeUs));
  ...
  private synchronized void releaseExcessFramesInAllSecondaryStreams() {
    for (int sequenceIndex = 0; sequenceIndex < inputSources.size(); sequenceIndex++) {
      if (sequenceIndex == PRIMARY_SEQUENCE_INDEX) {
        continue;
      }
      releaseExcessFramesInSecondaryStream(inputSources.valueAt(sequenceIndex));
    }
  }
  ...
  private synchronized void releaseExcessFramesInSecondaryStream(InputSource secondaryInputSource) {
    InputSource primaryInputSource = inputSources.get(PRIMARY_SEQUENCE_INDEX);
  ...
  private synchronized void maybeComposite()
    ...
    InputFrameInfo primaryInputFrame = framesToComposite.get(PRIMARY_SEQUENCE_INDEX);
  ...
  private synchronized ImmutableList<InputFrameInfo> getFramesToComposite() {
    ...
    InputFrameInfo primaryFrameToComposite =
        inputSources.get(PRIMARY_SEQUENCE_INDEX).frameInfos.element();
    framesToComposite.add(primaryFrameToComposite);

    for (int i = 0; i < inputSources.size(); i++) {
      if (i == PRIMARY_SEQUENCE_INDEX) {
        continue;
      }
