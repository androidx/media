...
public interface VideoCompositor extends GlTextureProducer {
   ...
   */
  void registerInputSource(@IntRange(from = 0) int sequenceIndex);

  /**
   * Signals that no more frames will come from the upstream {@link GlTextureProducer.Listener}.
   *
   * @param sequenceIndex The sequence index of the input source.
   */
  void signalEndOfInputSource(int sequenceIndex);

  /**
   * Queues an input texture to be composited.
   *
   * @param sequenceIndex The index of the input sequence, the same index used when {@linkplain
