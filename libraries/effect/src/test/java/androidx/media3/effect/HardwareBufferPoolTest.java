...
public final class HardwareBufferPoolTest {
  ...
  public void get_upToCapacity_createsNewFrames() {
    HardwareBufferPool pool = new HardwareBufferPool(CAPACITY, directExecutor(), unused -> {});
    Set<HardwareBuffer> seenBuffers = new HashSet<>();

    for (int i = 0; i < CAPACITY; i++) {
      HardwareBufferPool.HardwareBufferWithFence result = pool.get(DEFAULT_FORMAT, () -> {});
  ...
  public void get_afterRecycle_reusesBufferForSameFormat() {
    ...
    HardwareBufferPool.HardwareBufferWithFence result1 = pool.get(DEFAULT_FORMAT, () -> {});
    HardwareBuffer buffer1 = result1.hardwareBuffer;
    pool.recycle(buffer1, result1.acquireFence);

    HardwareBufferPool.HardwareBufferWithFence result2 = pool.get(DEFAULT_FORMAT, () -> {});
  ...
  public void get_afterRecycle_createsNewBufferForDifferentFormat() {
    ...
    HardwareBufferPool.HardwareBufferWithFence result2 = pool.get(format2, () -> {});
    assertThat(result2.hardwareBuffer).isNotSameInstanceAs(buffer1);
    assertThat(buffer1.isClosed()).isTrue();
  }

  @Test
  public void get_clearsIncompatibleBuffersFromFront() {
    HardwareBufferPool pool = new HardwareBufferPool(CAPACITY, directExecutor(), unused -> {});
    HardwareBufferFrameQueue.FrameFormat formatB =
        new HardwareBufferFrameQueue.FrameFormat.Builder().setWidth(200).setHeight(200).build();

    HardwareBufferPool.HardwareBufferWithFence result1 = pool.get(DEFAULT_FORMAT, () -> {});
    HardwareBufferPool.HardwareBufferWithFence result2 = pool.get(formatB, () -> {});
    HardwareBuffer buffer1 = result1.hardwareBuffer;
    ...
    HardwareBufferPool.HardwareBufferWithFence resultFrame = pool.get(formatB, () -> {});
  ...
  public void get_atCapacity_returnsNull() {
    ...
    for (int i = 0; i < CAPACITY; i++) {
      assertThat(pool.get(DEFAULT_FORMAT, () -> {})).isNotNull();
    }
    assertThat(pool.get(DEFAULT_FORMAT, () -> {})).isNull();
  }
  ...
  public void get_atCapacity_notifiesWakeupListenerOnReturn() {
    ...
    for (int i = 0; i < CAPACITY; i++) {
      results[i] = pool.get(DEFAULT_FORMAT, () -> {});
    }

    AtomicBoolean wakeupCalled = new AtomicBoolean(false);
    assertThat(pool.get(DEFAULT_FORMAT, () -> wakeupCalled.set(true))).isNull();
