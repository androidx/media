...
public final class DefaultVideoFrameProcessorPixelTest {
  ...
  @Test
  public void noOpEffect_withImageInputAndDefaultSettings_matchesGoldenFile() throws Exception {
    videoFrameProcessorTestRunner =
        getDefaultFrameProcessorTestRunnerBuilder(testId)
            // Use default SDR working color space.
            .setEffects(NO_OP_EFFECT)
            .build();
    Bitmap originalBitmap = readBitmap(IMAGE_JPG_ASSET_PATH);
    // VideoFrameProcessor may recycle the original bitmap, so we need a copy for comparison.
    Bitmap expectedBitmap = originalBitmap.copy(Bitmap.Config.ARGB_8888, /* isMutable= */ false);
