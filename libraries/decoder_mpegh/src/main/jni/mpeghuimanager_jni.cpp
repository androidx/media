...
extern "C" JNIEXPORT jint JNI_OnLoad(JavaVM* vm, void* reserved) {
  ...
  jclass clazz = env->FindClass("androidx/media3/decoder/mpegh/MpeghUiManagerJni");
  if (clazz == nullptr) {
    return -1;
  }
...
UIMANAGER_FUNC(jint, update, jobject inData, jint inDataLen,
  ...
  if (updateResult == MPEGH_UI_OK) {
    ...
    while (!(flagsOut & MPEGH_UI_NO_CHANGE)) {
      ...
      MPEGH_UI_ERROR stateResult = mpegh_UI_GetXmlSceneState(
          ctx->handle, ctx->xmlScratchBuffer.data(), XML_BUFFER_SIZE, flagsIn,
          &flagsOut);
