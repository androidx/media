 ...
 * priority) the `dataSpec`, [setRequestProperty] and the default parameters used to construct
 * the instance.
 */
class KtorDataSource private constructor(
     ...
     * @param scope A [CoroutineScope] for running suspend functions. If not provided, a default
     *   scope with [Dispatchers.IO] and a [SupervisorJob] will be created.
     */
    class Factory(
        private val httpClient: HttpClient,
         ...
         *
         * The default is `null`, which causes the default user agent of the underlying [HttpClient]
         * to be used.
         *
         * @param userAgent The user agent that will be used, or `null` to use the default user
         *   agent of the underlying [HttpClient].
         * @return This factory.
         */
         ...
         * The default is `null`.
         *
         * @param cacheControl The cache control header value that will be used, or `null` to clear
         ...
         * The default is `null`.
         *
         * @param contentTypePredicate The content type [Predicate], or `null` to clear a predicate
    ...
    override fun open(dataSpec: DataSpec): Long {
        ...
        try {
            ...
            this.responseInputStream =
                executeSuspend { httpResponse.bodyAsChannel().toInputStream() }
        } catch (e: IOException) {
        ...
        if (dataSpec.length != C.LENGTH_UNSET.toLong()) {
            ...
            bytesToRead =
                if (contentLength >= 0) contentLength - bytesToSkip else C.LENGTH_UNSET.toLong()
        }
    ...
    private fun executeRequest(dataSpec: DataSpec): HttpResponse {
        ...
        defaultRequestProperties?.snapshot?.forEach { (key, value) ->
            mergedHeaders[key] = value
        }
        requestProperties.snapshot.forEach { (key, value) ->
            mergedHeaders[key] = value
        }
        ...
        return executeSuspend {
            httpClient.prepareRequest {
                ...
                method = when (dataSpec.httpMethod) {
