...
If your application only needs to play http(s) content, using the Ktor extension
is as simple as updating any `DataSource.Factory` instantiations in your
application code to use `KtorDataSource.Factory`. If your application also needs
to play non-http(s) content such as local files, use: `new
DefaultDataSourceFactory( ... /* baseDataSourceFactory= */ new
