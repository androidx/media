...
public final class FrameExtractorInternal {
  ...
  @Nullable private Long pendingSeekPositionMs;
  ...
  public ListenableFuture<FrameExtractor.Frame> submitTask(FrameExtractionRequest request) {
    return executionSequencer.submitAsync(
        () -> {
          SettableFuture<FrameExtractor.Frame> taskCompleter = SettableFuture.create();
          ListenableFuture<FrameExtractor.Frame> workFuture = processTask(request);

          Futures.addCallback(
              workFuture,
              new FutureCallback<FrameExtractor.Frame>() {
                @Override
                public void onSuccess(FrameExtractor.Frame result) {
                  lastSeekDedupeFrame = result;
                  activeTaskCompleter.set(null);
                  taskCompleter.set(result);
                }

                @Override
                public void onFailure(Throwable t) {
                  activeTaskCompleter.set(null);
                  taskCompleter.setException(t);
                }
              },
              playerHandler::post);

          taskCompleter.addListener(() -> workFuture.cancel(false), directExecutor());
    ...
    ListenableFuture<Void> unused =
        executionSequencer.submit(
            () -> {
              ...
              decoderCountersSettableFuture.set(counters);
              return null;
            },
            playerHandler::post);
  ...
  private ListenableFuture<FrameExtractor.Frame> processTask(FrameExtractionRequest request) {
    return CallbackToFutureAdapter.getFuture(
        completer -> {
          ...
          ensurePlayerInitialized(request);
          ...
          if (!request.mediaItem.equals(player.getCurrentMediaItem())
              || player.getPlayerError() != null) {
            lastSeekDedupeFrame = null;
            pendingSeekPositionMs = request.positionMs;
  ...
  private void ensurePlayerInitialized(FrameExtractionRequest request) {
    if (player == null
  ...
  private static final class PlayerListener implements AnalyticsListener {
    ...
    public void onPlayerError(EventTime eventTime, PlaybackException error) {
      CallbackToFutureAdapter.Completer<FrameExtractor.Frame> frameBeingExtractedCompleter =
          checkNotNull(internal.activeTaskCompleter.get());
    ...
    public void onPlaybackStateChanged(EventTime eventTime, @Player.State int state) {
      if (state == Player.STATE_READY) {
        // Handle the deferred seek that was posted when the player was preparing.
        if (internal.pendingSeekPositionMs != null) {
          checkNotNull(internal.player).seekTo(internal.pendingSeekPositionMs);
          internal.pendingSeekPositionMs = null;
          return;
        }
  ...
  private static final class FrameReadingGlShaderProgram extends PassthroughShaderProgram {
    ...
    public void queueInputFrame(
      ...
      CallbackToFutureAdapter.Completer<FrameExtractor.Frame> frameBeingExtractedCompleter =
          checkNotNull(internal.activeTaskCompleter.get());
      frameBeingExtractedCompleter.set(
          new FrameExtractor.Frame(usToMs(presentationTimeUs), bitmap));
