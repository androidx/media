 ...
 *
 * Despite [kotlinx.coroutines.flow.callbackFlow] being recommended for a multi-shot API (like [Player]'s), a
 * [suspendCancellableCoroutine] is a lower-level construct that allows us to overcome the
 * limitations of [kotlinx.coroutines.flow.Flow]'s buffered dispatch. In our case, we will not be waiting for a particular
 ...
 * controls the timing of dispatching events to the caller more tightly than [kotlinx.coroutines.flow.Flow]s. Such timing
