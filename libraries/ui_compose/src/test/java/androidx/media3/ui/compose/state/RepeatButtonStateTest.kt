...
class RepeatButtonStateTest {
  ...
  @Test
  fun playerRepeatModeChanged_buttonRepeatModeChanged() = runComposeUiTest {
    val player = FakePlayer()

    lateinit var state: RepeatButtonState
    setContent { state = rememberRepeatButtonState(player = player) }
  ...
  fun onClick_stateBecomesDisabled_isNoOp() = runComposeUiTest {
    ...
    val spyPlayer = mock(Player::class.java, delegatesTo<Player>(player))
    lateinit var state: RepeatButtonState
    setContent { state = rememberRepeatButtonState(spyPlayer) }
  ...
  fun playerSetRepeatModeAndOnClick_inTheSameHandlerMessage_uiStateSynchronises() =
    runComposeUiTest {
      ...
      // RepeatButtonState changing its state in sync with the relevant Player events. This means
      // that we should never find ourselves in a situation where a button's icon (here: determined
      // by RepeatButtonState.repeatModeState) is out of sync with the Player's repeat mode. It can
  ...
  fun nullPlayer_buttonStateIsDisabled() = runComposeUiTest {
    lateinit var state: RepeatButtonState
    setContent { state = rememberRepeatButtonState(player = null) }
