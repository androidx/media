  ...
  public static class LicenseServer implements MediaDrmCallback {
    ...
    public KeyResponse executeProvisionRequest(UUID uuid, ProvisionRequest request)
      ...
      if (Arrays.equals(request.getData(), FAKE_PROVISION_REQUEST.getData())) {
        return new KeyResponse(Bytes.toArray(VALID_PROVISION_RESPONSE), LOAD_EVENT_INFO);
      } else {
        return new KeyResponse(Util.EMPTY_BYTE_ARRAY, LOAD_EVENT_INFO);
      }
    }
    ...
    public KeyResponse executeKeyRequest(UUID uuid, KeyRequest request)
      ...
      return new KeyResponse(Bytes.toArray(response), LOAD_EVENT_INFO);
    }
  }
