...
public class AdTimelineTest {
  ...
  @Test
  public void getPeriod_singlePeriodWithWindowOffset_doesNotShiftAdGroupTimes() {
    // Single period live stream with window offset 10s into the period.
    FakeTimeline contentTimeline =
        new FakeTimeline(
            new FakeTimeline.TimelineWindowDefinition.Builder()
                .setPeriodCount(1)
                .setWindowPositionInFirstPeriodUs(10 * MICROS_PER_SECOND)
                .setDurationUs(20 * MICROS_PER_SECOND)
                .build());

    long adGroupTimeUs = 15 * MICROS_PER_SECOND;
    AdPlaybackState adPlaybackState = new AdPlaybackState("adsId", adGroupTimeUs);

    AdTimeline adTimeline = new AdTimeline(contentTimeline, adPlaybackState);
    Period period = new Period();

    adTimeline.getPeriod(/* periodIndex= */ 0, period);

    // Ad group time is unchanged relative to period 0.
    assertThat(period.adPlaybackState.getAdGroup(0).timeUs).isEqualTo(adGroupTimeUs);
  }

  @Test
  public void getPeriod_multiPeriodWithWindowOffset_offsetsAdGroupTimesRelativeToPeriod0() {
    // Multi-period live stream: Period 0 (20s duration), Period 1 (20s duration),
    // Window starts 10s into Period 0.
    FakeTimeline contentTimeline =
        new FakeTimeline(
            new FakeTimeline.TimelineWindowDefinition.Builder()
                .setPeriodCount(2)
                .setWindowPositionInFirstPeriodUs(10 * MICROS_PER_SECOND)
                .setDurationUs(40 * MICROS_PER_SECOND)
                .build());

    long adGroup0TimeUs = 5 * MICROS_PER_SECOND; // In Period 0
    long adGroup1TimeUs =
        35 * MICROS_PER_SECOND; // In Period 1 (5s into Period 1, which starts at 30s)
    AdPlaybackState adPlaybackState = new AdPlaybackState("adsId", adGroup0TimeUs, adGroup1TimeUs);

    AdTimeline adTimeline = new AdTimeline(contentTimeline, adPlaybackState);
    Period period0 = new Period();
    Period period1 = new Period();

    adTimeline.getPeriod(/* periodIndex= */ 0, period0);
    adTimeline.getPeriod(/* periodIndex= */ 1, period1);

    // Period 0 retains ad group 0 at 5s.
    assertThat(period0.adPlaybackState.getAdGroup(0).timeUs).isEqualTo(adGroup0TimeUs);
    // Period 1 offsets ad group 1 by Period 0 duration (30s), so 35s - 30s = 5s.
    assertThat(period1.adPlaybackState.getAdGroup(1).timeUs).isEqualTo(5 * MICROS_PER_SECOND);
  }
}
