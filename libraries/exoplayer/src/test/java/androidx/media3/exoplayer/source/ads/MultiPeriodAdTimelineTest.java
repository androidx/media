...
public class MultiPeriodAdTimelineTest {
  ...
  public void getPeriod() {
    ...
    assertEquals(1, period1.getAdGroupIndexForPositionUs(1 * MICROS_PER_SECOND));
    assertEquals(2, period1.getAdGroupIndexAfterPositionUs(1 * MICROS_PER_SECOND));
    assertEquals(2, period1.getAdGroupIndexForPositionUs(10 * MICROS_PER_SECOND));
    assertEquals(3, period1.getAdGroupIndexAfterPositionUs(10 * MICROS_PER_SECOND));
    assertEquals(3, period1.getAdGroupIndexForPositionUs(19 * MICROS_PER_SECOND));
    // no more ads to be played in 1st period
    assertEquals(INDEX_UNSET, period1.getAdGroupIndexAfterPositionUs(19 * MICROS_PER_SECOND));

    Period period2 = new Period();
    multiPeriodAdTimeline.getPeriod(2, period2);

    // positions within the 2nd period
    assertEquals(3, period2.getAdGroupIndexForPositionUs(1 * MICROS_PER_SECOND));
    assertEquals(4, period2.getAdGroupIndexAfterPositionUs(1 * MICROS_PER_SECOND));
    assertEquals(4, period2.getAdGroupIndexForPositionUs(10 * MICROS_PER_SECOND));
    assertEquals(5, period2.getAdGroupIndexAfterPositionUs(10 * MICROS_PER_SECOND));
    assertEquals(5, period2.getAdGroupIndexForPositionUs(19 * MICROS_PER_SECOND));
    // no more ads to be played in 2nd period
    assertEquals(INDEX_UNSET, period2.getAdGroupIndexAfterPositionUs(19 * MICROS_PER_SECOND));
  }
}
