...
public class ExoPlayerAdTest {
  ...
  @Test
  public void playAds_liveStreamWithWindowOffset_adPlaysAtCorrectPosition() throws Exception {
    Timeline liveContentTimeline =
        new FakeTimeline(
            new TimelineWindowDefinition.Builder()
                .setPeriodCount(1)
                .setLive(true)
    ...
    AdsMediaSource adsMediaSource =
        new AdsMediaSource(
            new FakeMediaSource(liveContentTimeline, ExoPlayerTestRunner.VIDEO_FORMAT),
            new DataSpec(Uri.EMPTY),
            "adsId",
            /* adMediaSourceFactory= */ new FakeMediaSourceFactory(
                new TimelineWindowDefinition.Builder()
    ...
    player.play();
    // Play past the ad at window position 5s (period position 15s) and ad duration (5s).
    advance(player).untilPositionAtLeast(/* mediaItemIndex= */ 0, /* positionMs= */ 6_000L);
    player.release();

    ArgumentCaptor<PositionInfo> oldPositionsCaptor = ArgumentCaptor.forClass(PositionInfo.class);
    ArgumentCaptor<PositionInfo> newPositionsCaptor = ArgumentCaptor.forClass(PositionInfo.class);
    ArgumentCaptor<Integer> reasonsCaptor = ArgumentCaptor.forClass(Integer.class);
    verify(mockListener, atLeast(2))
        .onPositionDiscontinuity(
            oldPositionsCaptor.capture(), newPositionsCaptor.capture(), reasonsCaptor.capture());
    ...
    // Content to ad transition at window position 5s (period position 15s)
    assertThat(oldPositions.get(0).adGroupIndex).isEqualTo(C.INDEX_UNSET);
    assertThat(oldPositions.get(0).positionMs).isEqualTo(5_000L);
    assertThat(oldPositions.get(0).contentPositionMs).isEqualTo(5_000L);
    assertThat(newPositions.get(0).adGroupIndex).isEqualTo(0);
    assertThat(newPositions.get(0).positionMs).isEqualTo(0L);

    // Ad to content transition at ad completion (window position 5s)
    assertThat(oldPositions.get(1).adGroupIndex).isEqualTo(0);
    assertThat(oldPositions.get(1).positionMs).isEqualTo(5_000L); // Ad duration
    assertThat(oldPositions.get(1).contentPositionMs).isEqualTo(5_000L);
    assertThat(newPositions.get(1).adGroupIndex).isEqualTo(C.INDEX_UNSET);
  ...
  private static final class PrerollMeasurement {
    ...
    private final int prerollSamplesSkipped;
    private final int prerollSamplesRendered;
  ...
  private static final class PrerollEmittingMediaSource extends FakeMediaSource {
    ...
    private PrerollEmittingMediaSource(
  ...
  private static final class PrerollTrackingVideoRenderer extends FakeVideoRenderer {
    ...
    private int prerollSamplesSkipped;

    /** Preroll samples that would have been rendered, i.e. dropped or shown as stale content. */
    private int prerollSamplesRendered;

    private long streamStartPositionUs;

    private PrerollTrackingVideoRenderer(
