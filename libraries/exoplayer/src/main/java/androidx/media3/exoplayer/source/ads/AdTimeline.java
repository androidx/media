 ...
 * periods.
 *
 * <p>The ad group times in the input {@link AdPlaybackState} must be relative to the start of the
 * first {@link Timeline.Period} (i.e. {@code timeUs = 0} corresponds to the start of period 0). For
 * each period a modified {@link AdPlaybackState} is created:
 *
 * <ul>
 *   <li>Ad group time is offset relative to the period's start time
 *   <li>Post-roll ad group is kept only for the last period
...
public final class AdTimeline extends ForwardingTimeline {
   ...
   *     relative to the start of the first {@link Timeline.Period} (i.e. {@code timeUs = 0}
   *     corresponds to the start of the first period).
   */
  public AdTimeline(Timeline contentTimeline, AdPlaybackState adPlaybackState) {
    ...
    long period0PositionInWindowUs = period.positionInWindowUs;
    for (int periodIndex = 0; periodIndex < periodCount; periodIndex++) {
      timeline.getPeriod(periodIndex, period);
      adPlaybackStates[periodIndex] =
          forPeriod(
              adPlaybackState,
              period.positionInWindowUs - period0PositionInWindowUs,
              period.durationUs,
