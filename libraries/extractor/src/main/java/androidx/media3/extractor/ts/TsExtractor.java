...
public final class TsExtractor implements Extractor {
  ...
  private boolean updatePresentationLabels(List<AudioPresentation> aps,
    ...
    for (ListIterator<AudioPresentation> itr = aps.listIterator(); itr.hasNext(); ) {
      ...
      if (messageId == valueIfKeyNotFound) {
        Log.v(TAG, "No message id detected");
        return false;
      }
  ...
  private class SdtSectionReader implements SectionPayloadReader {
    ...
    public void consume(ParsableByteArray sectionData) {
     ...
     boolean gotLabels = updatePresentationLabels(audioPresentations, presentationLabels);
     if (gotLabels) {
       Log.d(TAG,"Audio presentations with labels: " + audioPresentations);
     }
     if (mode != MODE_HLS) {
      ...
      while (data.getPosition() < descriptorsEndPosition) {
        ...
        if (descriptorTag == TS_PMT_DESC_REGISTRATION) { // registration_descriptor
          ...
          if (descriptorTagExt == TS_PMT_DESC_DVB_EXT_AC4) {
            ...
            boolean lookingForLabels = false;
